docker-build:
	...
	trivy image 739561048503.dkr.ecr.us-east-1.amazonaws.com/auth-service:$(image_tag) -s CRITICAL,HIGH --ignore-unfixed
...
argocd-deploy:
	...
	argocd app create auth-service --sync-policy auto --upsert --repo https://github.com/raghudevopsb88/wmp-helm-v2.git --path . --dest-server https://kubernetes.default.svc   --dest-namespace default --helm-set-string image_tag=$(image_tag) --values values/auth-service.yml
